  ...
  return (
    <div>
      <h2 id="cart-heading" data-cy="CartHeading">
        <Translate contentKey="larissStoreApp.cart.home.title">Carts</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="larissStoreApp.cart.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/cart/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="larissStoreApp.cart.home.createLabel">Create new Cart</Translate>
          </Link>
        </div>
      </h2>
        ...
        {cartList && cartList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="larissStoreApp.cart.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="larissStoreApp.cart.customer">Customer</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {cartList.map((cart, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{cart.customer ? <Link to={`/customer/${cart.customer.id}`}>{cart.customer.firstName}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="larissStoreApp.cart.home.notFound">No Carts found</Translate>
            </div>
          )
        )}
      </div>
