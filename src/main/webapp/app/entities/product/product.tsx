  ...
  return (
    <div>
      <h2 id="product-heading" data-cy="ProductHeading">
        <Translate contentKey="larissStoreApp.product.home.title">Products</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="larissStoreApp.product.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/product/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="larissStoreApp.product.home.createLabel">Create new Product</Translate>
          </Link>
        </div>
      </h2>
        ...
        {productList && productList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="larissStoreApp.product.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('productName')}>
                  <Translate contentKey="larissStoreApp.product.productName">Product Name</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('productName')} />
                </th>
                <th className="hand" onClick={sort('description')}>
                  <Translate contentKey="larissStoreApp.product.description">Description</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('description')} />
                </th>
                <th className="hand" onClick={sort('currencyCode')}>
                  <Translate contentKey="larissStoreApp.product.currencyCode">Currency Code</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('currencyCode')} />
                </th>
                <th>
                  <Translate contentKey="larissStoreApp.product.category">Category</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {productList.map((product, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`larissStoreApp.CurrencyCode.${product.currencyCode}`} />
                  </td>
                  <td>{product.category ? <Link to={`/category/${product.category.id}`}>{product.category.categoryName}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="larissStoreApp.product.home.notFound">No Products found</Translate>
            </div>
          )
        )}
      </div>
